...
herdr agent history show <session-id> [--offset N] [--limit N] [--json]
herdr agent history refresh|status [--json]
herdr agent resume <session-id> [--agent claude] [--cwd PATH] [--workspace] [--no-focus] [--json]
```

`agent resume` reopens a session listed by `agent history`: a new tab in the workspace whose checkout matches the session's project (or a new workspace when none is open, or always with `--workspace`), running `claude --resume <id>` through your shell. A session that is already open in a pane is focused instead. `--cwd` overrides the project directory and is required for sessions the index does not know.

`agent history` searches past agent sessions indexed from Claude Code transcripts. Results are grouped by project directory and ranked with title matches first (`T`, including `/rename` titles), then first-prompt matches (`P`), then transcript text matches (`~`, shown with a snippet). Without a query it lists recent sessions. `show` prints the conversation of one session (user and assistant messages, oldest first). `--no-deep` skips transcript text, `--limit` caps the number of sessions, and `--json` prints the raw `agent_history.search` response. `refresh` rescans transcripts incrementally and `status` shows index size, last scan date, and the cache directory. See `[agent_history]` in [Configuration](/docs/configuration/) to disable indexing or text caching.
