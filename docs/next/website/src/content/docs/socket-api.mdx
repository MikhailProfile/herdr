...
| Agent | `agent.list`, `agent.get`, `agent.read`, `agent.explain`, `agent.send_keys`, `agent.prompt`, `agent.wait`, `agent.rename`, `agent.focus`, `agent.start`, `agent.resume`, `agent.view.set`, `agent.view.clear` |
| Agent history | `agent_history.search`, `agent_history.messages`, `agent_history.status`, `agent_history.refresh` |
...
`agent_history.search` searches past coding-agent sessions indexed from their transcript directories (Claude Code today). Pass `query` (all whitespace-separated terms must match), optional `limit` (default 200), and `deep` (default `true`) to include cached transcript text. Results are grouped by project directory; each session carries `match_tier` (`title` for `/rename` or generated titles, `prompt` for the first prompt, `text` for transcript text with a `snippet`), `open_pane_id` when a live pane already hosts that session, and the group carries `workspace_id` when an open workspace matches the project. `agent_history.messages` returns the user and assistant text of one indexed session (`agent`, `session_id`, optional `offset` and `limit`, default 400 messages, each cut at 8000 characters with `truncated` set) for previews. `agent_history.status` reports index size and whether a scan is running; `agent_history.refresh` schedules an incremental rescan and returns the same status. All three return `agent_history_disabled` when `[agent_history] enabled = false`.

`agent.resume` reopens a past session found by `agent_history.search` (Claude Code only today). Pass `session_id`, optional `cwd` (defaults to the project directory from the index), `focus` (default `true`), and `placement` (`tab` opens a new tab in the workspace whose checkout matches the project and creates a workspace when none matches; `workspace` always creates one). Herdr starts a shell in that directory and submits `claude --resume <id>` through it, so shell aliases and wrappers apply, and records the session reference on the pane so it survives server restarts. If a live pane already hosts the session, the response reports `focused_existing: true` and no new pane is created. Errors: `unsupported_agent`, `invalid_session_id`, `session_unknown` (not indexed and no `cwd`), `project_missing`, `tab_create_failed`, `resume_input_failed`.
